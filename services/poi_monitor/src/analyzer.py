...
class PoiAnalyzer:
    ...
    def analyze_pois(self, deployment_id: str, block_number: int) -> Optional[Dict]:
        ...
        try:
            ...
            if self.db.check_notification_sent(deployment_id, block_number):
                logger.debug(f"Already notified about {deployment_id} at block {block_number}")
                return None
            ...
            if not poi_submissions:
                logger.debug(f"No POI submissions found for {deployment_id} at block {block_number}")
                return None
                
            # If there's only one POI hash, there's no discrepancy
            if len(poi_submissions) == 1:
                logger.debug(f"All indexers agree on POI for {deployment_id} at block {block_number}")
                return None
    ...
    def _check_poi_reuse(self, submissions: Dict[str, Set[str]]) -> Dict[str, List[str]]:
        ...
        try:
                with self.db.get_connection() as conn:
                    with conn.cursor() as cur:
                        ...
                        poi_occurrences = {}
                        for row in results:
                            (poi_hash, deployment_id, block_number, 
                            indexer_addr, network, timestamp) = row
            ...
            for deployment_id, block_number in recent_submissions:
                try:
                    ...
                    if discrepancy:
                        ...
                        message = self.notifier.format_poi_discrepancy_message(discrepancy)
                    ...
                    logger.error(f"Error processing submission for deployment {deployment_id} at block {block_number}: {str(e)}", exc_info=True)
                    continue
